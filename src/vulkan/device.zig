...
pub const PhysicalDevice = struct {
    ...
    pub fn select(alloc: std.mem.Allocator, instance: c.VkInstance, surface: ?c.VkSurfaceKHR) !PhysicalDevice {
        ...
        const physical_devices = try alloc.alloc(c.VkPhysicalDevice, physical_device_count);
    ...
    fn is_physical_device_suitable(alloc: std.mem.Allocator, device: PhysicalDevice, surface: ?c.VkSurfaceKHR) !bool {
        ...
        if (surface) |surf| {
            const swapchain_support = try SwapchainSupportInfo.init(alloc, device.handle, surf);
        ...
        if (required_device_extensions.len > 0) {
            ...
            const device_extensions = try alloc.alloc(c.VkExtensionProperties, device_extension_count);
...
pub const Device = struct {
    ...
    pub fn create(alloc: std.mem.Allocator, physical_device: PhysicalDevice) !Device {
        ...
        try queue_family_set.put(alloc, physical_device.graphics_queue_family, {});
        try queue_family_set.put(alloc, physical_device.present_queue_family, {});
        try queue_family_set.put(alloc, physical_device.compute_queue_family, {});
        try queue_family_set.put(alloc, physical_device.transfer_queue_family, {});
        var qfi_iter = queue_family_set.iterator();
        try queue_create_infos.ensureTotalCapacity(alloc, queue_family_set.count());
        while (qfi_iter.next()) |qfi| {
            try queue_create_infos.append(alloc, std.mem.zeroInit(c.VkDeviceQueueCreateInfo, .{
        ...
        try check_vk(c.vkCreateDevice(physical_device.handle, &device_info, alloc_cb, &device));
