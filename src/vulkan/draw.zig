...
const FrameData = @import("framebuffer.zig").check_vk;
...
pub fn draw(core: *Core) void {
    ...
    const frame_index = core.framebuffer.current;
    const frame = core.framebuffer.frames[frame_index];
    ...
    // frame.buffer_deletion_queue.flush(core.gpu_allocator);
    // frame.frame_descriptors.clear_pools(core.device);
    ...
    // core.draw_extent.width = @intFromFloat(@as(f32, @floatFromInt(@min(self.swapchain_extent.width, self.draw_image_extent.width))) * self.render_scale);
...
// pub fn async_submit(self: *Self, submit_ctx: anytype) void {
//     comptime {
